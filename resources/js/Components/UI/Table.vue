...
function toggleRow(row) {
  const idx = localSelected.value.findIndex(r => r[props.rowKey] === row[props.rowKey])
...
function isSelected(row) {
  return localSelected.value.some(r => r[props.rowKey] === row[props.rowKey])
}
...
<template>
  <div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
      ...
      <table class="w-full">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th v-if="props.actions.length" class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Aksi</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
          <tr v-if="props.loading" class="animate-pulse">
            <td :colspan="props.columns.length + (props.selectable ? 1 : 0) + (props.actions.length ? 1 : 0)" class="px-4 py-8 text-center text-gray-500">Memuat...</td>
          </tr>
          <tr v-else-if="!props.loading && (!props.data || props.data.length === 0)">
            <td :colspan="props.columns.length + (props.selectable ? 1 : 0) + (props.actions.length ? 1 : 0)" class="px-4 py-8 text-center text-gray-500">{{ props.emptyMessage }}</td>
          </tr>
          <tr v-for="row in sortedData" :key="row[props.rowKey]"
            :class="['transition-colors', props.hoverable ? 'hover:bg-gray-50' : '', isSelected(row) ? 'bg-indigo-50' : '']"
          >
            ...
            <td v-for="col in props.columns" :key="col.key" class="px-4 py-3 text-sm text-gray-900" :class="col.align ? `text-${col.align}` : ''">
              <span v-if="col.render" v-html="col.render(row, col)"></span>
              <span v-else>{{ row[col.key] }}</span>
            </td>
            <td v-if="props.actions.length" class="px-4 py-3 text-center">
              <div class="flex items-center justify-center gap-1.5">
                <template v-for="act in props.actions" :key="act.label">
                  <button
    ...
    <div v-if="props.pagination" class="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
      <div class="text-sm text-gray-600">
        Menampilkan {{ (props.pagination.page - 1) * props.pagination.perPage + 1 }} - {{ Math.min(props.pagination.page * props.pagination.perPage, props.pagination.total) }} dari {{ props.pagination.total }}
      </div>
      <div class="flex gap-2">
        <Button size="sm" variant="outline" :disabled="props.pagination.page === 1" @click="$emit('page-change', props.pagination.page - 1)">Sebelumnya</Button>
        <Button size="sm" variant="outline" :disabled="props.pagination.page * props.pagination.perPage >= props.pagination.total" @click="$emit('page-change', props.pagination.page + 1)">Selanjutnya</Button>
      </div>
    </div>
  </div>
</template>