...
const statusColor = { draft: 'bg-gray-100 text-gray-700', counting: 'bg-blue-100 text-blue-700', review: 'bg-yellow-100 text-yellow-700', approved: 'bg-green-100 text-green-700', posted: 'bg-purple-100 text-purple-700', cancelled: 'bg-red-100 text-red-700' }

const columns = [
  { key: 'count_number', label: 'No. Count', width: 140 },
  { key: 'user.name', label: 'Petugas', width: 120 },
  { key: 'status', label: 'Status', width: 100, align: 'center', render: (row) => `<span class="px-2 py-0.5 rounded text-xs ${statusColor[row.status] || ''}">${row.status}</span>` },
...
const actions = [
  { label: 'Detail', icon: EyeIcon, variant: 'ghost', onClick: (row) => router.visit(`/stock-counts/${row.id}`) },
...
const filteredData = computed(() => {
  let data = props.stockCounts.data
  if (search.value) {
    const q = search.value.toLowerCase()
    data = data.filter(s => s.count_number?.toLowerCase().includes(q))
  }
...
async function createSC() {
  loading.value = true
  try {
    await router.post('/api/v1/stock-counts', form.value, {
      onSuccess: () => { showModal.value = false; success('Stock count dibuat'); router.reload() },
...
function openCountModal(sc) {
  countingSC.value = sc
  countInputs.value = {}
  sc.items?.forEach(item => { countInputs.value[item.id] = '' })
    ...
    <Card>
      <div class="flex flex-col sm:flex-row gap-3 mb-4">
        <div class="flex-1 relative">
          ...
          <input v-model="search" type="text" placeholder="Cari count number..." class="w-full pl-10 pr-4 py-2 bg-card border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
        </div>
        <Select v-model="statusFilter" :options="[{value:'',label:'Semua'},{value:'draft',label:'Draft'},{value:'counting',label:'Counting'},{value:'review',label:'Review'},{value:'approved',label:'Approved'},{value:'posted',label:'Posted'}]" class="w-36" />
      </div>
      <Table
        :columns="columns"
        :data="filteredData"
        ...
        :pagination="{
          ...
          onChange: (p) => router.visit('/stock-counts', { page: p }, { replace: true })
        }"
        emptyMessage="Belum ada stock count"
      />
    </Card>

    <Modal v-model="showModal" title="Buat Stock Count Baru" @confirm="createSC" :loading="loading">
      <div class="space-y-4">
        <div class="flex items-center gap-2">
          <input type="checkbox" v-model="form.is_blind" id="is_blind" class="w-4 h-4 text-primary rounded" />
          <label for="is_blind" class="text-sm">Blind Count (sembunyikan system qty dari petugas)</label>
        </div>
        <Select v-model="form.product_ids" :options="props.products.map(p=>({value:p.id,label:p.name + ' (Stok: ' + p.stock + ')'}))" multiple label="Produk (kosong = semua)" />
        <Input v-model="form.notes" type="textarea" label="Catatan" rows="2" />
      </div>
    </Modal>

    <Modal v-model="showCountModal" :title="'Input Hasil: ' + (countingSC?.count_number || '')" size="xl" @confirm="saveCount" :loading="loading">
      <div class="space-y-3 max-h-[60vh] overflow-y-auto">
        <div v-for="item in countingSC?.items" :key="item.id" class="bg-muted rounded-lg p-3">
          ...
          <input v-model.number="countInputs[item.id]" type="number" min="0" placeholder="Hasil hitung fisik" class="w-full px-3 py-2 border border-border rounded-lg text-sm" />
        </div>
      </div>
    </Modal>
  </AppLayout>
