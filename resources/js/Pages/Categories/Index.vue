...
const isLaras = ref(true)

function openCreate() { form.value = { name: '', slug: '' }; showModal.value = true }
async function save() {
  if (!form.value.name) { error('Nama wajib'); return }
  const url = isLaras.value ? '/api/v1/categories' : '/api/v1/categories'
  await router.post(url, { name: form.value.name, slug: form.value.name.toLowerCase().replace(/\s+/g,'-') }, { onSuccess: () => { showModal.value=false; success('Kategori dibuat'); router.reload() } })
}
...
<template>
  <AppLayout>
    ...
    <Card>
      ...
      <Table :columns="columns" :data="categories.data" emptyMessage="Belum ada kategori" />
    ...
    <Modal v-model="showModal" title="Tambah Kategori" @confirm="save">
      <div class="space-y-3">
